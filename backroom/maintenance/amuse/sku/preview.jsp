...
%>
				<table width="550" border="0" cellspacing="1" cellpadding="2">
					...
					<input type='hidden' name='pricem' value='<%= sku.pricem %>' />
					<input type='hidden' name='taxm' value='<%= sku.taxm %>' />
					<input type='hidden' name='pricec' value='<%= sku.pricec %>' />
					<input type='hidden' name='taxc' value='<%= sku.taxc %>' />
          <input type='hidden' name='sending' value='<%= sku.sending %>' />
          <input type='hidden' name='sending_tax' value='<%= sku.sending_tax %>' />
          <input type='hidden' name='minorder' value='<%= sku.minorder %>' />
          <input type='hidden' name='maxorder' value='<%= sku.maxorder %>' />
					<tr bgcolor="#999999">
          ...
          <tr bgcolor="#CCCCCC">
            ...
            <td  class="b" align="center" width="110">価格(税/送料込み)</td>
            <td  class="b" align="center" width="110">価格(税/送料込み)</td>
            <td  class="b" align="center" width="110">配送料(税/送料込み)</td>
            <td  class="b" align="center" width="110">手配数(最大/最小)</td>
          </tr>
          <tr bgcolor="#FFFFFF">
            ...
            <td  class="b" align="left" width="110">
          	...
          	<%= pf.ToPriceFormat( sku.tax ) %>
          	</td>
          	<td  class="b" align="left" width="110">
          	<%= pf.ToPriceFormat( sku.pricem ) %>/
          	<%= pf.ToPriceFormat( sku.taxm ) %>
          	</td>
          	<td  class="b" align="left" width="110">
          	<%= pf.ToPriceFormat( sku.pricec ) %>/
          	<%= pf.ToPriceFormat( sku.taxc ) %>
          	</td>
          	</td>
          	<td  class="b" align="left" width="110">
          	<%= pf.ToPriceFormat( sku.sending ) %>/
          	<%= pf.ToPriceFormat( sku.sending_tax ) %>
          	</td>
          	<td  class="b" align="left" width="110">
          	<%= sku.minorder %>/
          	<%= sku.maxorder %>
