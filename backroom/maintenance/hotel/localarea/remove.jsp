...
<%
	...
	LocalAreaSupNbr temp = new LocalAreaSupNbr();
	temp = temp.getRequestByUpdate( request );
	boolean resultFlag = handlersupnbr.removeLocalAreaSubNbr( temp );
...
<head>
<%	if( resultFlag ){	%>
<%=	jp.co.lastminute.maintenance.util.WebUtil.getAlert("íœ‚µ‚Ü‚µ‚½") %><%	}else{	%>
<%=	jp.co.lastminute.maintenance.util.WebUtil.getAlert("íœ‚É¸”s‚µ‚Ü‚µ‚½") %><%	}	%>
