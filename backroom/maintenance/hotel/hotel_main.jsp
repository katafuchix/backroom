...
<%
  ...
  //カテゴリー
	...
	Hotel pagelist = new Hotel();
	pagelist.init( Datasouce );
	//
	int pagetype_cd = type_cd * 100 + jp.co.lastminute.maintenance.Property.INDEX_;
  ...
  <tr>
    <td>
      <table border="0" cellspacing="1" cellpadding="2" width="100%">
        <tr align="center">
          <td colspan="5" bgcolor="#FFFFFF">ホテル管理画面</td>
        </tr>
        <%
		int end = jp.co.lastminute.maintenance.hotel.Property.pagemetrix[0][0];
		//for(int i=0; i<end; i++){
		for(int i=0; i<6; i++){

			Mentlist mentlist = null;
			Categoryselect select = null;
			Catid_small small = null;
			if( i < vc1.size() ){
				mentlist = (Mentlist)vc1.get( i );
				select = mentlist.getSelect();
				small = mentlist.getSmall();
			}else{
				mentlist = new Mentlist();
				select = new Categoryselect();
				select.startdate = Integer.parseInt( df.getNowDate(0, true) );
				small = new Catid_small();
			}
	//		boolean fg = true;
	//		if( fg )	return;
...
%>
        ...
        <tr>
          ...
          <input type="text" name="description" size="80" maxlength='40' value="<%= select.getDescription() %>">
          </td>
        </tr>
        <tr>
