...
<%	//セッションにて検索条件を取得
		...
		//リスト検索
		TicketSkuhandler handler = new TicketSkuhandler();
		...
		TicketSku[] skus = handler.getList( condition );
		TicketSku newsku = new TicketSku();
		try{
...
%>
	<form action="/backroom/maintenance/ticket/sku/productInserterCtr.jsp" method="post">
...
<%
		...
		for(int x = 0; x < skus.length; x++){
			TicketSku sku = skus[x];
%>
				<form action="/backroom/maintenance/ticket/sku/edit.jsp" method="post">
          <tr bgcolor="#FFFFFF">
            ...
            |<A HREF="/backroom/maintenance/ticket/allot/list.jsp?sku_id=<%= sku.sku_id %>">ALLOT</A>|
            </td>

            <td align="center" width="140" ><%= sku.salesfrom %>/<%= sku.salesto %><br/>
            |<A HREF="/backroom/maintenance/ticket/sku/delete.jsp?id=<%= sku.sku_id %>">データ完全削除</A>|</td>
