package jp.co.lastminute.maintenance.hotel.category;
 ...
 */
public class Category extends Commander implements CommandIF {
	/**
	 * コンテキストのリロード
	 */
/*
			...
			if (db.init(this.dataSource)) {
				...
				if (rows.size() > 0) {
					...
					for (int i = 0; i < rows.size(); i++) {
						category.put( "" + acceser.getData(i, 0) + "", acceser.getData(i, 1));
					}
					return category;
				}
			}
